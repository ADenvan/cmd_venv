...
urlpatterns = [
    ...
    path('', include('app.urls')), # Основное приложение
    path('api/', include('api.urls')), # API (Django REST Framework)
    path('api-auth/', include('rest_framework.urls')), # Вход в API через браузер (красивая форма логина)
    path('api-token-auth/', obtain_auth_token), # Получение токена аутентификации
]
